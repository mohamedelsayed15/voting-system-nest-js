...
export class PollController {
  ...
  async createVoter(@Body(new ValidationPipe()) body: PollDto) {
    try {
      ...
      for (const rival in rivals) {
        const insertedRival = await this.pollService.addRivalToPoll(
          rival,
          pollPk,
        );
  ...
  async voteToPoll(
    @Body(new ValidationPipe()) body: VoteToDto,
    @Request() req,
  ) {
    try {
      ...
      const logExist = await this.pollService.selectFromPollVotersLog(
        body.pollPk,
        user.pk,
      );

      // error 409 Conflict user already voted
      if (logExist) throw new ConflictException('user already voted');
      ...
      if (count === 0) throw new NotFoundException("couldn't find poll");
  ...
  async paginatePolls(
    @Query(new ValidationPipe()) query: PageQuery,
    @Request() req,
  ) {
    try {
      const user = req.user;
      ...
      const polls = await this.pollService.paginatePollToVoter(
        user.pk,
        10,
        offset,
      );
  ...
  async getResults(@Param() params: PollPk) {
    try {
      ...
      const totalVoters = results.reduce(
        (sum, rival) => sum + rival.votersNumber,
        0,
      );

      const resultsWithPercentage = results.map((rival) => {
        const percentage =
          totalVoters === 0 ? 0 : (rival.votersNumber / totalVoters) * 100;
        return {
          ...rival,
